...
def swapper(string , direction)
  if direction == 0
    ...
    string = string.tr("\\^AwWuvVmMn/\\\\" , "vVmMn\\^AwWu\\\\/")
  end
  return string
end

# Print border.
puts "+"+ "-"*(grid[0].size*2-1) + "+" + "-"*(grid[0].size*2-1) + "+"
...
2.times do
    ...
    grid.each do |row|
      tl = row
      tr = swapper(row[0,row.size-1],0).reverse
      new_row = tl+tr
    ...
    grid[0,grid.size-1].reverse.each do |row|
      bl = swapper(row,1)
      br = swapper(swapper(row[0,row.size-1],0),1).reverse
      new_row = bl + br
      new_grid << ["|"+new_row+"|" +swapper(new_row.reverse,0)+"|"]
    end
